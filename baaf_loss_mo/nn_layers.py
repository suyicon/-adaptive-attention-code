...
class BERT(nn.Module):
    def __init__(self, mod, input_size, block_size, d_model, N, heads, dropout, custom_attn=True, multclass = False,temp = 1.0):
        ...
        if mod == "trx":
            ...
            if multclass:
                self.out1 = nn.Linear(d_model, 2**block_size)
                self.out2 = nn.Linear(2**block_size, 1)
            else:
    ...
    def forward(self, src, mask, pe):
        ...
        if self.mod == "rec":
            belief = self.out1(enc_out)
            output = self.out2(belief)
            belief = F.softmax(belief/self.temp, dim=-1)
            return belief,output
        else:
            # encoders
            output = self.out1(enc_out)
...
def attention(q, k, v, d_k, attn_mask=None, dropout=None):
    ...
    # scores = F.softmax(scores[0][0], dim=-1)
    # attn = scores.cpu().detach().numpy()
