...
class RFC(nn.Module):
    def __init__(self, args):
        ...
        if self.args.reloc == 1:
            self.total_power_reloc = Power_reallocate_fine(args)
    ...
    def forward_train(self, belief_threshold, eachbatch, bVec, fwd_noise_par, ys, optimizer):
        ...
        for idx in range(self.truncated): # Go through parity bits
            ...
            parity = self.power_constraint(output,
                                           ...
                                           isTraining=1)
            ############# Power reallocation ###################################################
            reloc_weight = (self.args.numb_block / torch.sum(~mask, dim=-1)).unsqueeze(-1)
            parity = torch.where(mask.unsqueeze(-1), torch.zeros(self.args.batchSize, self.args.numb_block,1).to(self.args.device),
                                 (reloc_weight*parity.permute(1,0,2)).permute(1,0,2))
            ...
            train_log.append({"round":idx,"early_stop":early_stop.item()})
