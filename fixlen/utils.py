...
class Power_reallocate(torch.nn.Module):
    def __init__(self, args):
        ...
        if args.seq_reloc == 1:
            self.weight2 = torch.nn.Parameter(torch.Tensor(args.truncated, 1), requires_grad=True)
    ...
    def forward(self, inputs, seq_order):
        ...
        if self.args.seq_reloc == 1:
            self.wt2 = torch.sqrt(
                self.weight2 ** 2 * ((self.args.truncated) / torch.sum(self.weight2 ** 2)))
...
class Power_reallocate_fine(torch.nn.Module):
    def __init__(self, args):
        super(Power_reallocate_fine, self).__init__()
        self.args = args
        self.weight1 = torch.nn.Parameter(torch.Tensor(args.numb_block, 1), requires_grad=True)
        self.weight1.data.uniform_(1.0, 1.0)
        # if args.seq_reloc == 1:
    ...
    def forward(self, inputs,mask):
        ...
        # if self.args.seq_reloc == 1:
        #     inputs1 = inputs1 * self.wt2[seq_order]  # sequence_wise scaling
        weight2 = (self.args.numb_block/torch.sum(~mask,dim=-1)).unsqueeze(-1)
        inputs1=torch.where(mask,0,weight2*inputs1)
